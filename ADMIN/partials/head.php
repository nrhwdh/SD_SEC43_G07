...
<head>
  ...
  <title><?= h($page_title ?? 'The Pearl Admin') ?></title>
  ...
  <style>
    ...
    /* Prevent any accidental top gap/line */
    html, body { margin:0; padding:0; background:var(--sb-body); }

    body{ font-family:'Nunito',sans-serif; }

    .sb-wrap{ display:flex; min-height:100vh; }
    .sb-side{
      width:250px; background:linear-gradient(180deg,var(--sb-primary) 10%,#224abe 100%);
    ...
    .sb-nav .nav-link{
      color:#dbe5ff; padding:.65rem 1rem; border-radius:.35rem; margin:.25rem .75rem;
    ...
    .sb-top{
      background:#fff; border-bottom:1px solid var(--sb-gray-300); padding:.6rem 1rem;
    ...
    .sb-stat .num{ font-weight:700; font-size:1.35rem; }
    ...
    @media (max-width: 991px){
      .sb-side{ position:fixed; left:-270px; transition:left .25s; }
      .sb-side.open{ left:0; }
      .card{ margin-bottom:.75rem; }
    }

    /* Gradient login header */
    .login-hero{
      background:linear-gradient(180deg,#4e73df 0%, #224abe 100%);
      min-height:50vh; border-bottom-left-radius:2rem; border-bottom-right-radius:2rem;
    }
  </style>
</head>
<body>